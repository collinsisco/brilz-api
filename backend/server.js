...
app.get('/', (_req, res) => {
  res.json({ name: 'Brilz API 2.0', status: 'running', docs: '/health' });
});
...
app.use((error, _req, res, _next) => {
  ...
  if (error.message?.includes('CORS')) return res.status(403).json({ error: error.message });
  if (error.message?.includes('Only images')) return res.status(400).json({ error: error.message });
  res.status(500).json({ error: IS_PROD ? 'Internal server error' : error.message });
});
...
app.listen(PORT, () => {
  console.log(`
╔══════════════════════════════════════════╗
║   Brilz API 2.0 — Running               ║
╠══════════════════════════════════════════╣
║  Port:        ${PORT}                        ║
║  Environment: ${(process.env.NODE_ENV || 'development').padEnd(12)}            ║
║  Supabase:    ${process.env.SUPABASE_URL ? '✓ Connected' : '✗ Not configured'} ║
║  M-Pesa:      ${process.env.MPESA_CONSUMER_KEY ? '✓ Configured' : '✗ Not configured'}║
║  Email:       ${process.env.SMTP_PASS ? '✓ Configured' : '✗ Not configured'} ║
╠══════════════════════════════════════════╣
